...
import { TrendingUp, Target, Wallet, Activity, Flame, Snowflake, Trophy, Edit2, Rocket, Sparkles, Plus, ArrowRight, MessageCircle, Settings2, CheckCircle2, XCircle, CalendarDays } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* WHATSAPP BANNER */}
      <a href="https://whatsapp.com" target="_blank" rel="noopener noreferrer" className="block relative overflow-hidden rounded-2xl bg-gradient-to-r from-emerald-900 via-emerald-700 to-emerald-900 border border-emerald-500/50 group shadow-[0_0_20px_rgba(16,185,129,0.2)] hover:shadow-[0_0_30px_rgba(16,185,129,0.4)] transition-all duration-300 hover:-translate-y-1">
          <div className="absolute inset-0 bg-gradient-to-r from-transparent via-emerald-400/10 to-transparent -translate-x-[100%] group-hover:translate-x-[100%] transition-transform duration-1000 ease-in-out z-10"></div>
          <div className="absolute inset-0 opacity-20 bg-[url('https://upload.wikimedia.org/wikipedia/commons/thumb/6/6b/WhatsApp.svg/1200px-WhatsApp.svg.png')] bg-contain bg-center bg-no-repeat grayscale group-hover:grayscale-0 transition-all duration-500"></div>
          <div className="relative z-20 p-6 flex flex-col sm:flex-row items-center justify-between gap-6">
              <div className="flex items-center gap-5">
                   <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-[0_0_20px_rgba(255,255,255,0.3)] group-hover:scale-110 group-hover:rotate-6 transition-all duration-300">
                        <MessageCircle size={36} className="text-emerald-600 fill-emerald-600" />
                   </div>
                   <div className="text-center sm:text-left">
                       <h3 className="text-2xl sm:text-3xl font-display font-black text-white mb-1 tracking-tight drop-shadow-md">COMUNIDADE <span className="text-emerald-300">VIP</span></h3>
                       <p className="text-emerald-100 text-sm sm:text-base font-medium opacity-90">Resenha, Dicas Exclusivas e Greens em Tempo Real.</p>
                   </div>
              </div>
              <div className="flex items-center gap-2 bg-emerald-500 hover:bg-emerald-400 text-white px-8 py-3 rounded-xl font-bold shadow-lg group-hover:shadow-emerald-500/50 transition-all transform group-hover:scale-105 whitespace-nowrap">
                  ENTRAR AGORA <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" />
              </div>
          </div>
      </a>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="h-[300px] w-full">
                    {chartData.length > 0 ? (
                        ...
                        <AreaChart data={chartData}>
                            ...
                            <YAxis stroke="#64748b" fontSize={12} tickLine={false} axisLine={false} tickFormatter={(value) => `${value}u`} />
                            <Tooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#334155', color: '#f8fafc' }} itemStyle={{ color: '#22d3ee' }} formatter={(value: number) => [`${value} Unidades`, 'Lucro']} labelStyle={{ color: '#94a3b8' }} />
                            <Area type="monotone" dataKey="profit" stroke="#06b6d4" strokeWidth={2} fillOpacity={1} fill="url(#colorProfit)" />
                        </AreaChart>
                        </ResponsiveContainer>
                    ) : (
                        <div className="h-full flex flex-col items-center justify-center text-slate-500"><p>Sem dados suficientes.</p></div>
                    )}
                </div>
            </div>
        </div>
        ...
        <div className="lg:col-span-1">
            <div className="bg-gradient-to-br from-yellow-950/30 to-slate-900 border border-yellow-500/20 rounded-2xl p-5 shadow-lg shadow-yellow-900/5 relative overflow-hidden group hover:border-yellow-500/30 transition-colors h-full">
                ...
                <div className="flex justify-between items-start mb-4 relative z-10">
                    ...
                    <div className="bg-yellow-500/10 text-yellow-400 px-2 py-1 rounded text-xs font-bold border border-yellow-500/20 flex items-center gap-1"><CalendarDays size={10} /> Mês Atual</div>
                </div>
